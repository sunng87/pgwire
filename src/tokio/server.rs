    ...
    match socket.state() {
        ...
        PgWireConnectionState::CopyInProgress(is_extended_query) => {
            ...
            match message {
                ...
                // A Sync during copy-in means the client abandoned the copy
                // without sending CopyDone or CopyFail. PostgreSQL rejects
                // any message type other than CopyData/CopyDone/CopyFail
                // during COPY from stdin with a protocol violation; error
                // out instead of silently swallowing the message (which
                // deadlocks the client waiting for ReadyForQuery).
                PgWireFrontendMessage::Sync(_) => {
                    return Err(PgWireError::UserError(Box::new(ErrorInfo::new(
                        "ERROR".to_owned(),
                        "08P01".to_owned(),
                        "unexpected message type 0x53 (Sync) during COPY".to_owned(),
                    ))));
                }
                _ => {}
            }
...
mod copy_in_tests {
    ...
    use super::process_socket;
    use crate::api::Type;
    use crate::api::auth::{self, StartupHandler};
    use crate::api::copy::CopyHandler;
    use crate::api::portal::{Format, Portal};
    use crate::api::query::{ExtendedQueryHandler, SimpleQueryHandler};
    use crate::api::results::{CopyResponse, FieldInfo, Response, Tag};
    use crate::api::stmt::QueryParser;
    use crate::api::{ClientInfo, PgWireServerHandlers};
    use crate::error::{PgWireError, PgWireResult};
    use crate::messages::PgWireBackendMessage;
    use crate::messages::copy::{CopyData, CopyDone};
    use crate::messages::{PgWireBackendMessage as BackendMessage, PgWireFrontendMessage};
    ...
    impl StartupHandler for CopyInProcessor {
            ...
            PgWireError: From<<C as Sink<PgWireBackendMessage>>::Error>,
        {
            ...
            auth::finish_authentication(
                client,
                &crate::api::auth::DefaultServerParameterProvider::default(),
            )
            .await
        }
    }
